...
public class ToguzBoard {
    ...
    public boolean makeMove(int num, int playerColor) {
        ...
        if (num < 1 || num > 9 || toguzFields[numotau] == 0 || toguzFields[numotau] == 255) {
            return false; // Неверная лунка
        }
        ...
        if (toguzFields[numotau] == 3) {
            if ((playerColor == 0) && (toguzFields[20] == 0) && (numotau > 8) &&
                    (numotau < 17) && (toguzFields[21] != numotau - 8)) {
                toguzFields[18] += 3;
                toguzFields[numotau] = 255;
                toguzFields[20] = numotau - 8;
                capturedTuzdyk = true;
            } else if ((playerColor == 1) && (toguzFields[21] == 0) && (numotau < 8)
    ...
    public ArrayList<Integer> getAvailableMoves(int playerColor) {
